...
> 单人项目 · 纯前端 · GitHub Pages 部署 · v129 (2026-07-20)
...
css/styles.css  全部样式
index.html      入口（含 SW 注册 + 启动探针 + cache-buster ?v=129）
sw.js           Service Worker（HTML network-only / JS stale-while-revalidate）
...
## 关键约束（违反就会出 bug）
1. **每次升版本必须同步改 4 处**: `DATA_VERSION`/`_dataVersion`(app.js) + `?v=`(index.html 3 处) + `version.json` + `sw.js?v=`(index.html 探针块)
