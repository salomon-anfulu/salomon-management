#!/usr/bin/env python3
"""Parse 销售小票查询(40).XLSX and compute per-person performance data.
...
XLSX_PATH = "/Users/a86137/Downloads/销售小票查询 (40).XLSX"
...
WORK_HOURS = {
    '陈昕媛': 90.0, '杨子豪': 61.5, '李若彤': 82.5, '孔祥宇': 78.0,
    '王雅澜': 61.5, '龚赟昊': 54.0, '邓奇缘': 83.0, '王龙宇': 58.5,
    '何秋烨': 50.0, '田佳乐': 70.5, '朱凯赟': 58.5, '王靳毓': 58.0,
    '迟骋': 62.0,
}

WORK_DAYS = {
    '陈昕媛': 11, '杨子豪': 7, '李若彤': 10, '孔祥宇': 11,
    '王雅澜': 9, '龚赟昊': 7, '邓奇缘': 10, '王龙宇': 8,
    '何秋烨': 8, '田佳乐': 10, '朱凯赟': 8, '王靳毓': 7,
    '迟骋': 9,
}
...
def main():
    ...
    for r in all_rows:
        ...
        if row_type == '销售':
            ...
            # Remark format: 退换货(小票号:AMX...,销售员:AM...)
                ...
                if not name:
                    ...
                    for t, n in ticket_to_name.items():
                        if orig_ticket in t or t in orig_ticket:
                            ...
                            break
            if not name:
                # Fallback: match by remark abbreviation
                name = match_name(remark)
        ...
        # Amount already has correct sign (negative for returns)
    ...
    if untraced_returns:
        print(f"\n⚠️ Untraced returns/exchanges ({len(untraced_returns)}):")
    ...
    for name, d in sorted(data.items(), key=lambda x: -x[1]['sales']):
        ...
        for cat_name, cd in sorted(d['categories'].items(), key=lambda x: -x[1]['sales']):
            if cat_name == '其他' and cd['sales'] == 0:
                continue
            cat_sales = round(cd['sales'])
            cat_qty = cd['qty']
            pct = round(cat_sales / sales * 100, 1) if sales > 0 else 0
            if cat_sales > 0:  # Only include non-zero categories
    ...
    with open('/Users/a86137/Desktop/兼职/安福路兼职管理系统/data/june_perf_40.json', 'w', encoding='utf-8') as f:
        json.dump(result, f, ensure_ascii=False, indent=2)
    print(f"\nSaved to data/june_perf_40.json")
