#!/usr/bin/env python3
"""Generate JS code for june performance data block and ratings updates."""
import json
...
lines.append(f"        note: 'data as of 6/21 (file 40), net of returns -¥2,196 (1 traced) + 2 untraced from April',")
...
with open('/Users/a86137/Desktop/兼职/安福路兼职管理系统/data/june_block_v15.js', 'w', encoding='utf-8') as f:
    f.write(js_block)
...
for r in data['records']:
    ...
    if ho >= 300:
        ...
        perf_score = 1
    print(f"  {r['name']}: 时产¥{ho} → performance={perf_score} (¥{r['sales']:,}, {r['qty']}件, {r['tickets']}票)")
