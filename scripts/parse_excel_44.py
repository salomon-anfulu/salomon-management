#!/usr/bin/env python3
"""Parse 销售小票查询(44).XLSX and compute per-person performance data."""
import openpyxl
import json
import re
...
WORK_HOURS = {
    '陈昕媛': 100.5, '田佳乐': 76.5, '迟骋': 86.0, '王靳毓': 79.0,
    '朱凯赟': 75.0, '孔祥宇': 92.5, '邓奇缘': 89.0, '杨子豪': 76.0,
    '王雅澜': 78.0, '李若彤': 88.0, '王龙宇': 58.5, '何秋烨': 82.0,
    '龚赟昊': 86.0,
}

WORK_DAYS = {
    '陈昕媛': 13, '田佳乐': 10, '迟骋': 12, '王靳毓': 10,
    '朱凯赟': 10, '孔祥宇': 13, '邓奇缘': 11, '杨子豪': 9,
    '王雅澜': 11, '李若彤': 11, '王龙宇': 8, '何秋烨': 12,
    '龚赟昊': 11,
}
...
def main():
    ...
    for row in ws.iter_rows(min_row=3, values_only=True):
        ...
        all_rows.append({
            ...
            'qty': qty, 'remark': remark, 'category': category,
        })
    ...
    for name, d in sorted(data.items(), key=lambda x: -x[1]['sales']):
        ...
        records.append({
            'name': name, 'sales': sales, 'qty': qty, 'tickets': tickets,
            'avgPrice': avg_price, 'workHours': work_hours, 'workDays': work_days,
            'hourlyOutput': hourly_output, 'efficiency': efficiency,
            'salesShare': sales_share, 'categories': cats
        })
    ...
    with open('/Users/a86137/Desktop/兼职/安福路兼职管理系统/data/june_perf_44.json', 'w', encoding='utf-8') as f:
        json.dump(result, f, ensure_ascii=False, indent=2)
    print(f"\nSaved to data/june_perf_44.json")
